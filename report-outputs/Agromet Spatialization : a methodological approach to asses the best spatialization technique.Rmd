---
title: "Agromet Spatialization : a methodological approach to asses the '*best*' spatialization technique"
...
2.  __supervised machine learning [regression](https://math.stackexchange.com/questions/141381/regression-vs-classification) algorithms__ that given a set of data, find the best relationship that represents the set of data
...
# Defining the best supervised machine learning regression method
...
4. run a __benchmark experiment__ where __different desired regression learning methods__ are applied to various regression tasks (i.e. datasets with different combinations of explanatory variables + the target weather parameter) with the aim to compare and rank the combinations of methods + used explanatory variables using a [cross validation resampling strategy](https://www.researchgate.net/publication/226334863_Resampling_Strategies_for_Model_Assessment_and_Selection) (LOOCV) that provides the desired performance measures (e.g. RMSE, SE, MAE, etc)

---- 

5. Then __aggregate, by calculating the mean__, all the hourly performance measures on the whole representative subset to choose the regression learning method + regression task that globally performs the best
6. For each desired hourly dataset, apply the choosen combination of learning method + task to __build a model__ to make spatial predictions
...
## Which target dependent variables ? 
...
## Which R config and packages ? 

In order to ensure science reproductibility ([why it is important](https://www.sevenbridges.com/docker-based-solutions-to-reproducibility-in-science/)), the code is developed in a self-maintained and publicly availabe [Docker image](https://github.com/pokyah/agrometeorDocker)  
...
* The source code of the document is availbale on [github](https://github.com/pokyah/agrometeor-spatial-methodology)
